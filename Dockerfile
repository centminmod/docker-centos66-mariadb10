...
RUN yum -y install epel-release nano which perl-DBI python-setuptools; easy_install pip; pip install supervisor
...
RUN echo "exclude=*.i386 *.i586 *.i686 nginx* php* mysql*">> /etc/yum.conf; yum update -y
RUN mkdir -p /var/lib/mysql; groupadd mysql; useradd -r -g mysql mysql; chown -R mysql:mysql /var/lib/mysql; mkdir -p /home/mysqltmp; rm -rf /etc/my.cnf
...
RUN chmod +x /root/tools/mysqlstart; ls -lah /var/lib/mysql; tail -80 /var/log/mysqld.log
